...
g <- ggplot(iris, mapping = aes(x = Petal.Length, y = Petal.Width, color = Species)) + geom_point()
g

i <- iris %>%
  group_by(Species) %>%
  summarise(N = n()) %>%
  mutate(freq = 100*N/sum(N), petal = mean(iris$Petal.Length))
i
```
...
grafinho <- ggplot(dadinhos, aes(x = reorder(meses, indice), y = incerteza)) + geom_bar(stat = 'identity', width = 0.70, fill = 'pink') + labs(x = NULL, y = NULL, title = 'Indicador de incerteza na economia', subtitle = 'Em pontos') + geom_text(label = dadinhos$incerteza, nudge_y = 10) + theme(panel.background = element_rect(fill = 'white'), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1), panel.grid.major.y = element_line(colour = "black", size = 0.5)) + scale_y_continuous(labels = scales::number_format(accuracy = 0.1)) + geom_bar(data = filter(dadinhos, indice == 13), stat = 'identity', width = 0.70, fill = 'red')
